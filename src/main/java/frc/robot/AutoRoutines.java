  ...
  public Command autoSubCommand(String chosenAuto, int index) {
    ...
    */
    ...
    Command newStructure2 =
        Commands.sequence(
            new Intake(elevatorSubsystem, funnelSubsystem, tootsieSlideSubsystem, null)
                .onlyIf(startOrLeavingHPS),
            new SetElevatorLevel(
                elevatorSubsystem,
                ElevatorPositions
                    .safePosition, goRightBranch), // using L1 as the Safe Position because not sure if the "pos"
            // value in
            ...
            (pathGoesToHPS.getAsBoolean()
                ? new ParallelCommandGroup(
                    new SetElevatorLevel(elevatorSubsystem, ElevatorPositions.Intake, goRightBranch),
