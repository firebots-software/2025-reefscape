...
public class ApplicationMD4 extends SequentialCommandGroup {
  ...
  public ApplicationMD4(
    ...
    addCommands(
        ...
        new SetElevatorLevel(elevatorSubsystem, ElevatorPositions.L4, isScheduled()),
        new SetElevatorLevel(elevatorSubsystem, ElevatorPositions.L2, isScheduled()),
        new SetElevatorLevel(elevatorSubsystem, ElevatorPositions.L3, isScheduled()),
        new SetElevatorLevel(elevatorSubsystem, ElevatorPositions.L2, isScheduled()));

    addCommands(
        new ShootTootsieSlide(tootsieSlideSubsystem)
            .deadlineWith(new RunFunnelOutCommand(funnelSubsystem, null)));

    addCommands(new SetElevatorLevel(elevatorSubsystem, ElevatorPositions.L4, isScheduled()));

    addCommands(
        new SetElevatorLevel(elevatorSubsystem, ElevatorPositions.L2, isScheduled())
            .alongWith(new RunFunnelOutCommand(funnelSubsystem, null))
            .alongWith(new ShootTootsieSlide(tootsieSlideSubsystem)));
  }
}
